...
class Video:
    ...
    def crop(self, start_time, end_time, saveFile=False):
        ...
        save_path = (
            os.path.join(os.getcwd(), self.config.videos_dir, "processed") + ".mp4"
        )
    ...
    def createVideo(self):
        ...
        base_clip = ColorClip(
            size=(1080, 1920), color=[10, 10, 10], duration=self.clip.duration
        )
        bottom_meme_pos = (
            960 + (((1080 / self.clip.size[0]) * (self.clip.size[1])) / 2) + -20
        )
        if self.video_text:
            try:
                ...
                print(
                    "Please make sure that you have ImageMagick is not installed on your computer, or (for Windows users) that you didn't specify the path to the ImageMagick binary in file conf.py, or that the path you specified is incorrect"
                )
            ...
            self.clip = CompositeVideoClip(
                [
                    base_clip,
                    self.clip.set_position(("center", "center")),
        ...
        dir = (
            os.path.join(self.config.post_processing_video_path, "post-processed")
            + ".mp4"
        )
    ...
    def is_valid_file_format(self):
        if not self.source_ref.endswith(".mp4") and not self.source_ref.endswith(
            ".webm"
        ):
            exit(
                f"File: {self.source_ref} has wrong file extension. Must be .mp4 or .webm."
            )
    ...
    def get_youtube_video(self, max_res=True):
        ...
        valid_streams = sorted(
            streams, reverse=True, key=lambda x: x.resolution is not None
        )
        filtered_streams = sorted(
            valid_streams, reverse=True, key=lambda x: int(x.resolution.split("p")[0])
        )
        if filtered_streams:
            ...
            filename = os.path.join(
                os.getcwd(), Config.get().videos_dir, "pre-processed" + ".mp4"
            )
            return filename
        ...
        audio = (
            YouTube(url)
            .streams.filter(file_extension="webm", only_audio=True, adaptive=True)
            .first()
        )
        if video and audio:
            ...
            # print(resolution)
            if resolution >= 360:
                downloaded_v_path = video.download(
                    output_path=os.path.join(os.getcwd(), self.config.videos_dir),
                    filename=random_filename,
                )
                print("Downloaded Video File @ " + video.resolution)
                downloaded_a_path = audio.download(
                    output_path=os.path.join(os.getcwd(), self.config.videos_dir),
                    filename="a" + random_filename,
                )
                print("Downloaded Audio File")
                file_check_iter = 0
                while not os.path.exists(downloaded_a_path) and os.path.exists(
                    downloaded_v_path
                ):
                ...
                composite_video = VideoFileClip(downloaded_v_path).set_audio(
                    AudioFileClip(downloaded_a_path)
                )
