...
def convert_tags(text, session):
    ...
    def convert(match):
        ...
        if match.group(1):
            text_extra.append(
                text_extra_block(
                    end, end + len(match.group(1)) + 1, 1, match.group(1), "", str(i)
                )
            )
            ...
            user_id = r.text.split('webapp.user-detail":{"userInfo":{"user":{"id":"')[
                1
            ].split('"')[0]
            text_extra.append(
                text_extra_block(
                    end, end + len(match.group(2)) + 1, 0, "", user_id, str(i)
                )
            )
...
def getTagsExtra(title, tags, users, session):
    ...
    for user in users:
        ...
        text_extra.append(
            {
                "start": len(title) - len(verified_user) - 1,
                "end": len(title),
