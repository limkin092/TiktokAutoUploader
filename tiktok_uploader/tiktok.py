...
def upload_video(
    ...
    if schedule_time > 0:
        data["feature_common_info_list"][0]["schedule_time"] = schedule_time + int(
            time.time()
        )
    ...
    while True:
        ...
        if r.json()["status_code"] == 0:
            print(
                f"Published successfully {'| Scheduled for ' + str(schedule_time) if schedule_time else ''}"
            )
            uploaded = True
            break
        else:
...
def upload_to_tiktok(video_file, session):
    ...
    with open(
        os.path.join(os.getcwd(), Config.get().videos_dir, video_file), "rb"
    ) as f:
